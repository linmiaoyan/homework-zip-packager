...
class SoundManager:
    ...
    def generate_sounds(self):
        # 攻击音效：清脆的金属敲击声
        self.sounds['attack'] = pygame.mixer.Sound(self.create_attack_sound())
        # 受伤音效：短促的疼痛声
        self.sounds['hurt'] = pygame.mixer.Sound(self.create_hurt_sound())
        # 技能音效：魔法释放声
        self.sounds['skill'] = pygame.mixer.Sound(self.create_skill_sound())
        # 子弹音效：快速嗖嗖声
        self.sounds['bullet'] = pygame.mixer.Sound(self.create_bullet_sound())
        # 击杀音效：完成感
        self.sounds['kill'] = pygame.mixer.Sound(self.create_kill_sound())
        # 拾取音效：金币/物品声
        self.sounds['pickup'] = pygame.mixer.Sound(self.create_pickup_sound())
        # 护盾音效：能量冲击
        self.sounds['shield'] = pygame.mixer.Sound(self.create_shield_sound())
        # BOSS攻击：沉重打击
        self.sounds['boss_attack'] = pygame.mixer.Sound(self.create_boss_sound())
        # 影遁：神秘消失
        self.sounds['shadow'] = pygame.mixer.Sound(self.create_shadow_sound())
        # 翻滚：快速移动
        self.sounds['roll'] = pygame.mixer.Sound(self.create_roll_sound())
    
    def create_attack_sound(self):
        sr = 44100
        samples = int(sr * 0.15)
        wave = bytearray()
        for i in range(samples):
            ...
            p = t / 0.15
            # 快速攻击的包络 - 更具冲击力
            env = math.exp(-p * 7) if p < 0.3 else math.exp(-(p - 0.3) * 10) * 0.3
            # 丰富的金属敲击声 - 多个谐波
            v = math.sin(2 * math.pi * 420 * t) * 0.25
            v += math.sin(2 * math.pi * 840 * t) * 0.15
            v += math.sin(2 * math.pi * 1260 * t) * 0.08
            v += math.sin(2 * math.pi * 1680 * t) * 0.04
            # 添加轻微的敲击质感
            click = math.sin(2 * math.pi * 2500 * t) * 0.02 * math.exp(-p * 15)
            val = int((v + click) * 32767 * env * 0.32)
            wave.append((val >> 8) & 0xff)
            wave.append(val & 0xff)
        return bytes(wave)
    
    def create_hurt_sound(self):
        sr = 44100
        samples = int(sr * 0.2)
        wave = bytearray()
        for i in range(samples):
            ...
            # 疼痛声 - 快速下降的频率，有轻微的失真感
            f = 480 * (1 - p * 0.7)
            env = math.exp(-p * 4) * (0.8 + 0.2 * math.sin(p * math.pi * 3))
            v = math.sin(2 * math.pi * f * t) * 0.22
            v += math.sin(2 * math.pi * f * 1.3 * t) * 0.12
            # 添加轻微的噪音质感
            noise = (random.random() - 0.5) * 0.03 * math.exp(-p * 8)
            val = int((v + noise) * 32767 * env * 0.28)
            wave.append((val >> 8) & 0xff)
            wave.append(val & 0xff)
        return bytes(wave)
    
    def create_skill_sound(self):
        sr = 44100
        samples = int(sr * 0.3)
        wave = bytearray()
        for i in range(samples):
            ...
            # 魔法释放声 - 丰富的上升频率，带闪烁效果
            f = 220 * (1 + p * 1.2)
            env = math.pow(math.sin(p * math.pi), 1.2)
            # 主音
            v = math.sin(2 * math.pi * f * t) * 0.16
            v += math.sin(2 * math.pi * f * 1.26 * t) * 0.1
            v += math.sin(2 * math.pi * f * 1.58 * t) * 0.05
            # 添加闪烁效果
            shimmer = math.sin(2 * math.pi * 8 * t) * 0.03 * env
            # 添加轻微的魔法滋滋声
            magic_hum = math.sin(2 * math.pi * 60 * t) * 0.04
            val = int((v + shimmer + magic_hum) * 32767 * env * 0.3)
            wave.append((val >> 8) & 0xff)
            wave.append(val & 0xff)
        return bytes(wave)
    
    def create_bullet_sound(self):
        sr = 44100
        samples = int(sr * 0.08)
        wave = bytearray()
        for i in range(samples):
            ...
            env = math.exp(-p * 12)
            # 嗖嗖声
            v = math.sin(2 * math.pi * f1 * t) * 0.28
            v += math.sin(2 * math.pi * f2 * t) * 0.15
            # 添加轻微的空气感
            air = (random.random() - 0.5) * 0.02 * env
            val = int((v + air) * 32767 * env * 0.25)
            wave.append((val >> 8) & 0xff)
            wave.append(val & 0xff)
        return bytes(wave)
    
    def create_kill_sound(self):
        sr = 44100
        samples = int(sr * 0.35)
        wave = bytearray()
        for i in range(samples):
            ...
            if p < 0.3:
                ...
                f = 330 * 1.45 * (1 + (p - 0.3) * 0.8)
            else:
                f = 330 * 1.45 * 1.24 * (1 - (p - 0.6) * 0.6)
            env = math.exp(-p * 2.5) if p < 0.7 else math.exp(-(p - 0.7) * 5) * 0.6
            # 三和弦，营造胜利感
            v = math.sin(2 * math.pi * f * t) * 0.2
            v += math.sin(2 * math.pi * f * 1.25 * t) * 0.12
            v += math.sin(2 * math.pi * f * 1.5 * t) * 0.08
            val = int(v * 32767 * env * 0.32)
            wave.append((val >> 8) & 0xff)
            wave.append(val & 0xff)
        return bytes(wave)
    ...
    def create_pickup_sound(self):
        ...
        for i in range(samples):
            ...
            # 金币/拾取声 - 清脆的上升音
            f = 520 * (1 + p * 1.1)
            env = math.pow(math.sin(p * math.pi / 2), 1.3) if p < 0.6 else math.pow(1 - (p - 0.6) / 0.4, 2.2)
            # 金属碰撞声
            v = math.sin(2 * math.pi * f * t) * 0.22
            v += math.sin(2 * math.pi * f * 2.0 * t) * 0.12
            # 添加轻微的金属共振
            resonance = math.sin(2 * math.pi * f * 3.0 * t) * 0.04
            val = int((v + resonance) * 32767 * env * 0.3)
            wave.append((val >> 8) & 0xff)
            wave.append(val & 0xff)
        return bytes(wave)
    
    def create_shield_sound(self):
        sr = 44100
        samples = int(sr * 0.2)
        wave = bytearray()
        for i in range(samples):
            ...
            env = math.exp(-p * 5)
            # 低频冲击
            v = math.sin(2 * math.pi * f_base * t) * 0.25
            v += math.sin(2 * math.pi * f_base * 2.0 * t) * 0.14
            v += math.sin(2 * math.pi * f_base * 3.0 * t) * 0.07
            # 添加能量感
            energy = math.sin(2 * math.pi * f_base * 1.5 * t) * 0.05 * env
            val = int((v + energy) * 32767 * env * 0.3)
            wave.append((val >> 8) & 0xff)
            wave.append(val & 0xff)
        return bytes(wave)
    
    def create_boss_sound(self):
        sr = 44100
        samples = int(sr * 0.4)
        wave = bytearray()
        for i in range(samples):
            ...
            env = math.exp(-p * 2.2)
            # 低频为主，有震撼感
            v = math.sin(2 * math.pi * f_base * t) * 0.3
            v += math.sin(2 * math.pi * f_base * 2.0 * t) * 0.18
            v += math.sin(2 * math.pi * f_base * 3.0 * t) * 0.08
            # 添加轻微的失真感
            distort = math.copysign(0.02, v) * (1 - env)
            val = int((v + distort) * 32767 * env * 0.35)
            wave.append((val >> 8) & 0xff)
            wave.append(val & 0xff)
        return bytes(wave)
    
    def create_shadow_sound(self):
        sr = 44100
        samples = int(sr * 0.32)
        wave = bytearray()
        for i in range(samples):
            ...
            # 影遁声 - 神秘的下降频率，有阴影感
            f = 520 * (1 - p * 0.55)
            env = math.pow(math.sin(p * math.pi), 1.1) * 0.95
            # 神秘的音色
            v = math.sin(2 * math.pi * f * t) * 0.18
            v += math.sin(2 * math.pi * f * 1.33 * t) * 0.1
            v += math.sin(2 * math.pi * f * 1.66 * t) * 0.05
            # 添加阴影感的轻微效果
            shadow = math.sin(2 * math.pi * 12 * t) * 0.02 * env
            val = int((v + shadow) * 32767 * env * 0.28)
            wave.append((val >> 8) & 0xff)
            wave.append(val & 0xff)
        return bytes(wave)
    
    def create_roll_sound(self):
        sr = 44100
        samples = int(sr * 0.12)
        wave = bytearray()
        for i in range(samples):
            ...
            p = t / 0.12
            # 翻滚声 - 快速的嗖嗖声+风声质感
            f1 = 700 * (1 + math.sin(p * math.pi * 2) * 0.15)
            f2 = 1100 * (1 + math.sin(p * math.pi * 2 + 0.5) * 0.12)
            env = math.exp(-p * 10)
            v = math.sin(2 * math.pi * f1 * t) * 0.26
            v += math.sin(2 * math.pi * f2 * t) * 0.14
            # 添加轻微的风声
            wind = (random.random() - 0.5) * 0.025 * env
            val = int((v + wind) * 32767 * env * 0.28)
            wave.append((val >> 8) & 0xff)
            wave.append(val & 0xff)
        return bytes(wave)
...
class Player(pygame.sprite.Sprite):
    ...
    def melee_attack(self, angle, data, color_type='normal'):
        ...
        for merchant in merchants:
            dx = merchant.rect.centerx - self.rect.centerx
            dy = merchant.rect.centery - self.rect.centery
            dist = math.hypot(dx, dy)
            if dist <= data['range']:
                merchant_angle = math.atan2(dy, dx)
                angle_diff = abs(angle - merchant_angle)
                if angle_diff > math.pi:
                    angle_diff = 2 * math.pi - angle_diff
                if angle_diff < angle_range:
                    merchant.take_damage(data['damage'])
    ...
    def grapple_hook_attack(self, angle, data):
        ...
        if closest_target:
            if target_type == 'mob':
                closest_target.hp -= data['damage']
                pull_direction = math.atan2(self.rect.centery - closest_target.rect.centery, 
                                          self.rect.centerx - closest_target.rect.centerx)
                closest_target.rect.x += math.cos(pull_direction) * data.get('pull_distance', 80)
                closest_target.rect.y += math.sin(pull_direction) * data.get('pull_distance', 80)
                if closest_target.hp <= 0:
                    player.score += 10
                    spawn_pickups(closest_target.rect.centerx, closest_target.rect.centery, 1)
                    closest_target.kill()
            elif target_type == 'merchant':
                closest_target.take_damage(data['damage'])
    ...
    def nunchaku_attack(self, angle, data):
        ...
        for i in range(data['hit_count']):
            ...
            for merchant in merchants:
                ...
                if dist <= data['range']:
                    merchant_angle = math.atan2(dy, dx)
                    angle_diff = abs(hit_angle - merchant_angle)
                    if angle_diff > math.pi:
                        angle_diff = 2 * math.pi - angle_diff
                    if angle_diff < 1.2:
                        merchant.take_damage(data['damage'])
            ...
            for enemy in enemies:
                ...
                if dist <= data['range']:
                    enemy_angle = math.atan2(dy, dx)
                    angle_diff = abs(hit_angle - enemy_angle)
                    if angle_diff > math.pi:
                        angle_diff = 2 * math.pi - angle_diff
                    if angle_diff < 1.2:
                        enemy.take_damage(data['damage'])
    ...
    def use_skill(self):
        ...
        if now - self.last_skill_use > data['cooldown']:
            ...
            # 根据技能类型播放不同音效
            if skill_type == 'roll':
                sound_manager.play('roll', 0.5)
            elif skill_type == 'stealth':
                sound_manager.play('shadow', 0.4)
            else:
                sound_manager.play('skill', 0.4)
    ...
    def update(self):
        ...
        if self.laser:
            ...
            for merchant in merchants:
                ...
                dist = math.hypot(dx, dy)
                if dist < laser_length:
                    angle_to_merchant = math.atan2(dy, dx)
                    angle_diff = abs(laser_angle - angle_to_merchant)
                    if angle_diff > math.pi:
                        angle_diff = 2 * math.pi - angle_diff
                    if angle_diff < angle_threshold:
                        merchant.take_damage(self.damage)
                        hit = True
            
            for enemy in enemies:
                # 检查敌人（包括黄金雇佣兵）是否在激光路径上
                dx = enemy.rect.centerx - self.rect.centerx
                dy = enemy.rect.centery - self.rect.centery
                dist = math.hypot(dx, dy)
                if dist < laser_length:
                    angle_to_enemy = math.atan2(dy, dx)
                    angle_diff = abs(laser_angle - angle_to_enemy)
                    if angle_diff > math.pi:
                        angle_diff = 2 * math.pi - angle_diff
                    if angle_diff < angle_threshold:
            ...
            for merchant in merchants:
                if self.rect.colliderect(merchant.rect):
                    merchant.take_damage(self.damage)
                    hit = True
                    break
            ...
            for enemy in enemies:
                if self.rect.colliderect(enemy.rect):
                    enemy.take_damage(self.damage)
                    # 添加命中效果
                    effect = HitEffect(enemy.rect.centerx, enemy.rect.centery, self.damage)
                    all_sprites.add(effect)
                    hit_effects.add(effect)
                    hit = True
                    break
...
class Mob(pygame.sprite.Sprite):
    def __init__(self, mob_type='melee', level=1):
        ...
        if mob_type == 'melee':
            ...
            self.attack_range = 500  # 大幅增加攻击范围，远距离射击
        ...
        if self.mob_type == 'laser':
            self.aggro_range = 1200
        else:
            self.aggro_range = 500
        if self.mob_type == 'laser':
            self.retreat_range = 1300
        else:
            self.retreat_range = 600
...
class GoldenWave(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        for merchant in merchants:
            if merchant not in self.hit_mobs:
                dx = merchant.rect.centerx - self.x
                dy = merchant.rect.centery - self.y
                dist = math.hypot(dx, dy)
                angle_to_merchant = math.atan2(dy, dx)
                angle_diff = abs(angle_to_merchant - self.angle)
                if angle_diff > math.pi:
                    angle_diff = 2 * math.pi - angle_diff
                if dist < self.radius + merchant.rect.width // 2 and angle_diff < 1.3:
                    merchant.take_damage(self.damage)
                    self.hit_mobs.append(merchant)
        
        for enemy in enemies:
            if enemy not in self.hit_mobs:
                dx = enemy.rect.centerx - self.x
                dy = enemy.rect.centery - self.y
                dist = math.hypot(dx, dy)
                angle_to_enemy = math.atan2(dy, dx)
                angle_diff = abs(angle_to_enemy - self.angle)
                if angle_diff > math.pi:
                    angle_diff = 2 * math.pi - angle_diff
                if dist < self.radius + enemy.rect.width // 2 and angle_diff < 1.3:
                    enemy.take_damage(self.damage)
                    self.hit_mobs.append(enemy)
                    if enemy.hp <= 0:
                        player.gold += 5
                        enemy.kill()
        ...
        for bullet in enemy_bullets:
            ...
            dy = bullet.rect.centery - self.y
            dist = math.hypot(dx, dy)
            if dist < self.radius + bullet.rect.width // 2:
                bullet.kill()
...
class LaserEffect(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        for merchant in merchants:
            ...
            dy = merchant.rect.centery - self.y
            angle_to_merchant = math.atan2(dy, dx)
            angle_diff = abs(self.angle - angle_to_merchant)
            if angle_diff > math.pi:
                angle_diff = 2 * math.pi - angle_diff
            if angle_diff < angle_threshold:
                dist = math.hypot(dx, dy)
    ...
    def update(self):
        ...
        for merchant in merchants:
            ...
            if dist < 2000:
                ...
                dx_ref = merchant.rect.centerx - self.reflect_x
                dy_ref = merchant.rect.centery - self.reflect_y
                dist_ref = math.hypot(dx_ref, dy_ref)
                if dist_ref < 2000:
                    angle_to_merchant_ref = math.atan2(dy_ref, dx_ref)
                    angle_diff_ref = abs(angle_to_merchant_ref - self.reflect_angle)
                    if angle_diff_ref > math.pi:
                        angle_diff_ref = 2 * math.pi - angle_diff_ref
                    if angle_diff_ref < 0.3:
                        merchant.take_damage(self.damage)
        ...
        for enemy in enemies:
            ...
            if dist < 2000:
                ...
                dx_ref = enemy.rect.centerx - self.reflect_x
                dy_ref = enemy.rect.centery - self.reflect_y
                dist_ref = math.hypot(dx_ref, dy_ref)
                if dist_ref < 2000:
                    angle_to_enemy_ref = math.atan2(dy_ref, dx_ref)
                    angle_diff_ref = abs(angle_to_enemy_ref - self.reflect_angle)
                    if angle_diff_ref > math.pi:
                        angle_diff_ref = 2 * math.pi - angle_diff_ref
                    if angle_diff_ref < 0.3:
                        enemy.take_damage(self.damage)
...
class Boss(pygame.sprite.Sprite):
    ...
    def shoot(self):
        ...
        if now - self.last_attack > self.attack_cooldowns['shoot']:
            ...
            # 播放BOSS攻击音效
            sound_manager.play('boss_attack', 0.3)
...
class Merchant(pygame.sprite.Sprite):
    def __init__(self):
        ...
        # 优化物品列表 - 去除重复，添加更多有用物品
        all_items = [
            {'name': '手枪', 'price': 50, 'type': 'weapon'},
            {'name': '霰弹枪', 'price': 150, 'type': 'weapon'},
            {'name': '剑', 'price': 100, 'type': 'weapon'},
            {'name': '步枪', 'price': 200, 'type': 'weapon'},
            {'name': '狙击枪', 'price': 300, 'type': 'weapon'},
            {'name': '火箭炮', 'price': 500, 'type': 'weapon'},
            {'name': '回旋镖', 'price': 180, 'type': 'weapon'},
            {'name': '手里剑', 'price': 120, 'type': 'weapon'},
            {'name': '冰锥', 'price': 150, 'type': 'weapon'},
            {'name': '火球', 'price': 320, 'type': 'weapon'},
            {'name': '毒云', 'price': 290, 'type': 'weapon'},
            {'name': '圣剑', 'price': 280, 'type': 'weapon'},
            {'name': '黑骑士的剑', 'price': 350, 'type': 'weapon'},
            {'name': '咖喱棒', 'price': 300, 'type': 'weapon'},
            {'name': '双枪', 'price': 200, 'type': 'weapon'},
            {'name': '战斧', 'price': 180, 'type': 'weapon'},
            {'name': '飞刀', 'price': 80, 'type': 'weapon'},
            {'name': '能量剑', 'price': 220, 'type': 'weapon'},
            {'name': '激光剑', 'price': 180, 'type': 'weapon'},
            {'name': '链刃', 'price': 250, 'type': 'weapon'},
            {'name': '蓄力弓', 'price': 280, 'type': 'weapon'},
            {'name': '瞬光刺', 'price': 200, 'type': 'weapon'},
            {'name': '炽焰流', 'price': 260, 'type': 'weapon'},
            {'name': '灭核裁决', 'price': 450, 'type': 'weapon'},
            {'name': '双节棍', 'price': 100, 'type': 'weapon'},
            {'name': '暗影刃', 'price': 220, 'type': 'weapon'},
            {'name': '生命药水', 'price': 30, 'type': 'consumable'},
            {'name': '魔法药水', 'price': 20, 'type': 'consumable'},
            {'name': '大生命药水', 'price': 60, 'type': 'consumable'},
            {'name': '大魔法药水', 'price': 40, 'type': 'consumable'},
            {'name': '生命上限提升', 'price': 80, 'type': 'upgrade'},
            {'name': '魔力上限提升', 'price': 60, 'type': 'upgrade'},
            {'name': '攻击力提升', 'price': 100, 'type': 'upgrade'},
            {'name': '移动速度提升', 'price': 70, 'type': 'upgrade'}
        ]
    ...
    def draw_merchant(self):
        self.image.fill((0, 0, 0, 0))
        # 绘制帽子（商人特色）
        pygame.draw.rect(self.image, (80, 60, 30), (8, 0, 24, 6))
        pygame.draw.rect(self.image, (100, 75, 40), (12, 2, 16, 4))
        # 绘制头部（棕色）
        pygame.draw.rect(self.image, (255, 200, 0), (10, 6, 20, 19))
        # 绘制面部
        pygame.draw.rect(self.image, (255, 255, 200), (12, 8, 16, 14))
        # 绘制眼睛
        pygame.draw.circle(self.image, (0, 0, 0), (17, 13), 2)
        pygame.draw.circle(self.image, (0, 0, 0), (23, 13), 2)
        # 绘制眉毛
        pygame.draw.rect(self.image, (120, 90, 60), (15, 11, 4, 1))
        pygame.draw.rect(self.image, (120, 90, 60), (21, 11, 4, 1))
        # 绘制微笑嘴巴
        pygame.draw.arc(self.image, (0, 0, 0), (15, 15, 10, 6), 3.14, 0, 1)
        # 绘制胡须（更浓密的商人胡须）
        pygame.draw.rect(self.image, (200, 180, 160), (13, 19, 14, 6))
        pygame.draw.rect(self.image, (180, 160, 140), (15, 18, 10, 2))
        # 绘制身体（紫色长袍）
        pygame.draw.rect(self.image, (80, 60, 120), (8, 25, 24, 15))
        # 绘制装饰领结
        pygame.draw.rect(self.image, (150, 100, 50), (17, 25, 6, 6))
        pygame.draw.rect(self.image, (180, 120, 60), (18, 26, 4, 4))
        # 绘制手臂
        pygame.draw.rect(self.image, (255, 200, 0), (3, 26, 5, 9))
        pygame.draw.rect(self.image, (255, 200, 0), (32, 26, 5, 9))
        # 绘制钱袋（商人特色）
        pygame.draw.circle(self.image, (180, 140, 80), (6, 30), 4)
        pygame.draw.rect(self.image, (160, 120, 60), (5, 27, 2, 3))
        # 绘制腿部
        pygame.draw.rect(self.image, (60, 40, 100), (11, 35, 8, 5))
        pygame.draw.rect(self.image, (60, 40, 100), (21, 35, 8, 5))

    def update(self):
        # 简单的浮动动画
        self.animation_timer += 1
        float_offset = math.sin(self.animation_timer * 0.05) * 3
    ...
    def buy(self, index, player):
        if 0 <= index < len(self.items):
            ...
            if item['type'] == 'weapon' and item['name'] in player.weapons:
                print("已拥有该武器")
                return
            ...
            if player.gold >= item['price']:
                ...
                if item['type'] == 'consumable':
                    ...
                    elif item['name'] == '大生命药水':
                        player.hp = min(player.max_hp, player.hp + 100)
                    elif item['name'] == '魔法药水':
                        player.mp = min(player.max_mp, player.mp + 50)
                    elif item['name'] == '大魔法药水':
                        player.mp = min(player.max_mp, player.mp + 100)
                    ...
                    elif item['name'] == '魔力上限提升':
                        player.max_mp += 20
                        player.mp = min(player.mp + 20, player.max_mp)
                    elif item['name'] == '攻击力提升':
                        player.damage_multiplier = getattr(player, 'damage_multiplier', 1.0) + 0.1
                    elif item['name'] == '移动速度提升':
                        player.speed_base = getattr(player, 'speed_base', 5.5) + 0.5
                        player.speed = player.speed_base
                    print(f"购买了{item['name']}，属性已提升")
                ...
                # 播放购买音效
                sound_manager.play('pickup', 0.6)
            else:
                print("金币不足")
    
    def take_damage(self, damage):
        """商人受到伤害，变成敌对状态"""
        if not hasattr(self, 'hostile'):
            self.hostile = False
            self.hp = 0
        
        if not self.hostile:
            # 商人变成敌对Boss
            hostile_boss = HostileMerchantBoss(self.items)
            all_sprites.add(hostile_boss)
            enemies.add(hostile_boss)
            self.kill()
            sound_manager.play('hurt', 0.5)

class HostileMerchantBoss(pygame.sprite.Sprite):
    """敌对商人Boss：攻击商人后出现，击败后获得所有物品"""
    def __init__(self, items):
        ...
        # 阶段机制 - 先设置！
        self.phase = 1  # 1 = 第一阶段，2 = 第二阶段（狂暴）
        self.phase_triggered = False
        
        # Boss 属性（加强版）
        self.max_hp = 1600  # 大幅增加血量
        self.hp = self.max_hp
        self.speed = 2.6  # 更快的速度
        self.damage = 12  # 更高的伤害
        self.armor = 0.4  # 更高的护甲
        
        # 攻击相关
        self.last_attack = 0
        self.attack_cooldown = 280  # 更快的攻击间隔
        self.attack_mode = 'shoot'
        self.mode_timer = 0
        self.mode_duration = 100  # 更快的模式切换
        ...
        # 绘制图像 - 属性都设置好之后再绘制！
        self.image = pygame.Surface((40, 40), pygame.SRCALPHA)
        self.draw_hostile_merchant()
        self.rect = self.image.get_rect(center=(WIDTH//2, HEIGHT//2))
        self.animation_timer = 0
        self.original_y = self.rect.y
    
    def draw_hostile_merchant(self):
        """画愤怒的商人，支持阶段变化"""
        self.image.fill((0, 0, 0, 0))
        ...
        if self.phase == 1:
            ...
            # 第二阶段：狂暴阶段
            hat_color = (200, 0, 0)
            face_color = (255, 150, 120)
            body_color = (80, 20, 40)
        
        # 帽子
        pygame.draw.rect(self.image, hat_color, (8, 0, 24, 6))
        pygame.draw.rect(self.image, tuple(max(0, c - 30) for c in hat_color), (12, 2, 16, 4))
        # 头部
        pygame.draw.rect(self.image, (255, 200, 0), (10, 6, 20, 19))
        # 面部
        pygame.draw.rect(self.image, face_color, (12, 8, 16, 14))
        # 眼睛
        pygame.draw.circle(self.image, (200, 0, 0), (17, 13), 3 if self.phase == 1 else 4)
        pygame.draw.circle(self.image, (200, 0, 0), (23, 13), 3 if self.phase == 1 else 4)
        pygame.draw.circle(self.image, (0, 0, 0), (17, 13), 1)
        pygame.draw.circle(self.image, (0, 0, 0), (23, 13), 1)
        # 眉毛
        pygame.draw.polygon(self.image, (120, 90, 60), [(14, 10), (19, 8), (16, 12)])
        pygame.draw.polygon(self.image, (120, 90, 60), [(26, 10), (21, 8), (24, 12)])
        # 嘴巴
        mouth_size = (16, 18, 8, 3) if self.phase == 1 else (15, 17, 10, 5)
        pygame.draw.rect(self.image, (0, 0, 0), mouth_size)
        # 胡须
        pygame.draw.rect(self.image, (180, 160, 140), (13, 19, 14, 6))
        pygame.draw.rect(self.image, (160, 140, 120), (15, 18, 10, 2))
        # 身体
        pygame.draw.rect(self.image, body_color, (8, 25, 24, 15))
        # 领结
        pygame.draw.rect(self.image, (200, 50, 50), (17, 25, 6, 6))
        pygame.draw.rect(self.image, (220, 70, 70), (18, 26, 4, 4))
        # 手臂
        pygame.draw.rect(self.image, (255, 200, 0), (3, 26, 5, 9))
        pygame.draw.rect(self.image, (255, 200, 0), (32, 26, 5, 9))
        # 武器（钱袋）
        pygame.draw.circle(self.image, (160, 120, 60), (5, 32), 5)
        pygame.draw.rect(self.image, (140, 100, 40), (4, 28, 2, 4))
        # 腿部
        pygame.draw.rect(self.image, (50, 30, 80), (11, 35, 8, 5))
        pygame.draw.rect(self.image, (50, 30, 80), (21, 35, 8, 5))
        
        # 狂暴阶段特效：红色光晕
        if self.phase == 2:
            for i in range(3):
                alpha = 80 - i * 20
                pygame.draw.circle(self.image, (200, 50, 50, alpha), (20, 20), 20 - i * 5, 1)
    ...
    def update(self):
        ...
        # 检查阶段转换
        if self.hp <= self.max_hp * 0.4 and not self.phase_triggered:
            self.enter_phase_2()
        
        # 愤怒的浮动动画
        self.animation_timer += 1
        float_speed = 0.08 if self.phase == 1 else 0.15
        float_amplitude = 4 if self.phase == 1 else 6
        float_offset = math.sin(self.animation_timer * float_speed) * float_amplitude
        self.rect.y = self.original_y + float_offset
        
        # 模式切换
        self.mode_timer += 1
        mode_duration = self.mode_duration if self.phase == 1 else self.mode_duration * 0.7
    ...
    def enter_phase_2(self):
        ...
        sound_manager.play('boss_attack', 0.6)
        
        # 立即召唤一波雇佣兵
        self.summon_mercenaries()
    ...
    def summon_mercenaries(self):
        """召唤黄金雇佣兵"""
        num_mercenaries = 2 if self.phase == 1 else 4
        
        for i in range(num_mercenaries):
            # 在Boss周围随机位置生成
            angle = (i / num_mercenaries) * 2 * math.pi
            distance = 60 + random.randint(0, 40)
            x = self.rect.centerx + math.cos(angle) * distance
            y = self.rect.centery + math.sin(angle) * distance
            
            # 确保在房间内
            x = max(ROOM_X + 30, min(ROOM_X + ROOM_W - 30, x))
            y = max(ROOM_Y + 30, min(ROOM_Y + ROOM_H - 30, y))
            
            mercenary = GoldMercenary(x, y)
            all_sprites.add(mercenary)
    ...
    def switch_attack_mode(self):
        """切换攻击模式"""
        # 大幅增加激光模式的权重，确保经常触发
        modes = ['laser', 'laser', 'laser', 'laser', 'laser', 'shoot', 'throw_coins', 'gold_wave', 'chase', 'gold_storm']
        self.attack_mode = random.choice(modes)
    
    def execute_attack(self):
        now = pygame.time.get_ticks()
        
        if self.attack_mode == 'shoot':
            self.shoot_coins(now)
        elif self.attack_mode == 'throw_coins':
            self.throw_coins(now)
        elif self.attack_mode == 'gold_wave':
            self.gold_wave(now)
        elif self.attack_mode == 'chase':
            self.melee_attack(now)
        elif self.attack_mode == 'gold_storm':
            self.gold_storm(now)
        elif self.attack_mode == 'laser':
            self.laser_attack(now)
    
    def shoot_coins(self, now):
        """发射金币弹幕"""
        ...
        if now - self.last_attack > cooldown:
            ...
            num_bullets = 3 if self.phase == 1 else 5
            spread_range = 0.2 if self.phase == 1 else 0.25
            
            for i in range(num_bullets):
                spread = (i - (num_bullets - 1) / 2) * spread_range
                bullet = EnemyBullet(self.rect.centerx, self.rect.centery, 
                                   base_angle + spread, self.damage)
                bullet.speed = 10 if self.phase == 1 else 12
                all_sprites.add(bullet)
                enemy_bullets.add(bullet)
            
            sound_manager.play('bullet', 0.2)
    
    def throw_coins(self, now):
        """投掷大金币"""
        cooldown = self.attack_cooldown * 1.5
        if self.phase == 2:
            cooldown *= 0.7
        ...
        if now - self.last_attack > cooldown:
            ...
            damage = self.damage * 2 if self.phase == 1 else self.damage * 2.5
            bullet = EnemyBullet(self.rect.centerx, self.rect.centery, angle, damage)
            bullet.speed = 8 if self.phase == 1 else 10
            all_sprites.add(bullet)
            enemy_bullets.add(bullet)
            
            sound_manager.play('attack', 0.3)
    
    def gold_wave(self, now):
        """金币波浪攻击"""
        cooldown = self.attack_cooldown * 2
        if self.phase == 2:
            cooldown *= 0.6
        ...
        if now - self.last_attack > cooldown:
            ...
            num_waves = 7 if self.phase == 1 else 10
            for i in range(num_waves):
                angle = math.pi / 3 + (i * math.pi / 16)
                bullet = EnemyBullet(self.rect.centerx, self.rect.centery, angle, self.damage)
                bullet.speed = 9 if self.phase == 1 else 11
                all_sprites.add(bullet)
                enemy_bullets.add(bullet)
            
            sound_manager.play('skill', 0.3)
    
    def melee_attack(self, now):
        """近战攻击"""
        dist = math.hypot(player.rect.centerx - self.rect.centerx,
                         player.rect.centery - self.rect.centery)
        
        range_limit = 60 if self.phase == 1 else 75
        damage_multiplier = 1.5 if self.phase == 1 else 2.0
        
        if dist < range_limit and now - self.last_attack > self.attack_cooldown:
            self.last_attack = now
            player.take_damage(self.damage * damage_multiplier)
            sound_manager.play('hurt', 0.4)
    
    def gold_storm(self, now):
        """黄金风暴攻击（新技能）"""
        cooldown = self.attack_cooldown * 3
        if self.phase == 2:
            cooldown *= 0.5
        ...
        if now - self.last_attack > cooldown:
            ...
            # 全方位弹幕
            num_rays = 12 if self.phase == 1 else 16
            ...
            for i in range(30):
                ...
                speed = random.uniform(2, 5)
                particle = Particle(self.rect.centerx, self.rect.centery, 
                                   (255, 215, 0), 3, speed, 25)
                particle.dx = math.cos(angle) * speed
                particle.dy = math.sin(angle) * speed
                particles.add(particle)
            
            sound_manager.play('boss_attack', 0.5)
    
    def laser_attack(self, now):
        """商人激光攻击：金色激光！"""
        if self.laser_state != 'idle':
            return
        
        cooldown = self.attack_cooldown * 2.5
        if self.phase == 2:
            cooldown *= 0.6
        ...
        if now - self.last_attack > cooldown:
            ...
            for i in range(30):
                particle = Particle(self.rect.centerx, self.rect.centery,
                                  (255, 255, 0), random.randint(2, 5), 
                                  random.uniform(2, 6), random.randint(15, 30))
                particles.add(particle)
                
            sound_manager.play('skill', 0.4)
    
    def update_laser(self):
        """更新激光攻击状态（只处理状态机，伤害由主循环处理）"""
        if self.laser_state == 'idle':
            return
        ...
        if self.laser_state == 'charging':
            ...
            if self.laser_timer < 30:
                ...
                for i in range(2):
                    # 在激光线路上随机位置生成粒子
                    t = random.random()
                    x = self.laser_start[0] + (self.laser_end[0] - self.laser_start[0]) * t
                    y = self.laser_start[1] + (self.laser_end[1] - self.laser_start[1]) * t
                    
                    particle = Particle(x, y,
                                      (255, 255, 0), random.randint(2, 4), 
                                      random.uniform(1, 3), random.randint(10, 20))
                    particles.add(particle)
            else:
                # 进入发射阶段
                self.laser_state = 'firing'
                ...
                for i in range(40):
                    particle = Particle(self.rect.centerx, self.rect.centery,
                                      (255, 215, 0), random.randint(3, 6), 
                                      random.uniform(3, 7), random.randint(20, 35))
                    particles.add(particle)
            ...
            for i in range(3):
                ...
                particle = Particle(x, y,
                                  (255, 255, 200), random.randint(2, 4), 
                                  random.uniform(1, 2), random.randint(8, 15))
                particles.add(particle)
            
            # 等待结束
            if self.laser_timer >= 15:
                self.laser_state = 'idle'
                self.laser_damage = False
    ...
    def on_death(self):
        ...
        for item in self.items:
            # 物品作为金币/道具掉落
            if item['type'] == 'weapon' and item['name'] not in player.weapons:
                player.weapons.append(item['name'])
        ...
        sound_manager.play('kill', 0.6)
        self.kill()

class GoldMercenary(pygame.sprite.Sprite):
    """黄金雇佣兵：敌对商人召唤的小兵"""
    def __init__(self, x, y):
        ...
        # 攻击相关
        self.last_attack = 0
        self.attack_cooldown = 1500  # 1.5秒攻击间隔
    
    def draw_mercenary(self):
        """画黄金雇佣兵"""
        self.image.fill((0, 0, 0, 0))
        # 头盔（金色）
        pygame.draw.rect(self.image, (212, 175, 55), (6, 0, 18, 8))
        pygame.draw.rect(self.image, (180, 140, 30), (8, 2, 14, 4))
        # 眼睛位置
        pygame.draw.rect(self.image, (60, 40, 20), (10, 4, 10, 2))
        # 身体（金色盔甲）
        pygame.draw.rect(self.image, (212, 175, 55), (5, 8, 20, 14))
        pygame.draw.rect(self.image, (180, 140, 30), (7, 10, 16, 10))
        # 手臂
        pygame.draw.rect(self.image, (212, 175, 55), (1, 9, 4, 10))
        pygame.draw.rect(self.image, (212, 175, 55), (25, 9, 4, 10))
        # 武器（短剑）
        pygame.draw.rect(self.image, (160, 140, 100), (0, 14, 2, 6))
        pygame.draw.rect(self.image, (180, 160, 120), (0, 12, 2, 2))
        # 腿部
        pygame.draw.rect(self.image, (160, 120, 40), (8, 22, 6, 8))
        pygame.draw.rect(self.image, (160, 120, 40), (16, 22, 6, 8))
    
    def update(self):
        # 浮动动画
        self.animation_timer += 1
        float_offset = math.sin(self.animation_timer * 0.1) * 2
        self.rect.y = self.original_y + float_offset
        ...
        # 攻击玩家
        now = pygame.time.get_ticks()
        if dist_to_player < 40 and now - self.last_attack > self.attack_cooldown:
            self.last_attack = now
            player.take_damage(self.damage)
            sound_manager.play('hurt', 0.2)
...
class MimicChest(pygame.sprite.Sprite):
    """宝箱怪：看起来像普通宝箱，打开后变成敌对怪物！"""
    def __init__(self, x, y):
        super().__init__()
        # 初始看起来像普通宝箱
        self.image = pygame.Surface((30, 30))
        ...
        self.last_attack = 0
        self.opened = False  # 兼容Chest的opened属性

    def draw_awakened(self):
        """变成怪物后的外观"""
        self.image = pygame.Surface((35, 35))
        self.image.fill((139, 69, 19))  # 棕色
        # 画眼睛
        pygame.draw.circle(self.image, (255, 0, 0), (12, 12), 4)
        pygame.draw.circle(self.image, (255, 0, 0), (23, 12), 4)
        # 画嘴巴
        pygame.draw.rect(self.image, (0, 0, 0), (8, 22, 19, 8))
        # 画尖牙
        pygame.draw.rect(self.image, (255, 255, 255), (10, 22, 3, 4))
        pygame.draw.rect(self.image, (255, 255, 255), (22, 22, 3, 4))
    ...
    def open(self):
        ...
        if not self.awakened:
            ...
            self.opened = True
            self.draw_awakened()
            # 把自己添加到enemies组，同时从chests组移除
            all_sprites.add(self)
            enemies.add(self)
            # 从chests组移除
            if self in chests:
                chests.remove(self)
            sound_manager.play('skill', 0.5)
            return True  # 表示触发了动作
        return False
    ...
    def update(self):
        ...
        # 攻击玩家
        now = pygame.time.get_ticks()
        if dist < 40 and now - self.last_attack > self.attack_cooldown:
            player.take_damage(self.damage)
            self.last_attack = now
            # 攻击特效
            attack_effect = HitEffect(self.rect.centerx, self.rect.centery, self.damage)
            all_sprites.add(attack_effect)
    ...
    def take_damage(self, damage):
        ...
        for i in range(5):
            particle = Particle(self.rect.centerx, self.rect.centery,
                               (255, 0, 0), 3, random.uniform(2, 4), 20)
    ...
    def die(self):
        ...
        for i in range(20):
            particle = Particle(self.rect.centerx, self.rect.centery,
                               (139, 69, 19), 3, random.uniform(2, 5), 30)
            particle.dx = random.uniform(-3, 3)
            particle.dy = random.uniform(-3, 3)
            particles.add(particle)
        
        # 掉落一些金币
        for i in range(3):
            pickup = Pickup(self.rect.centerx + random.randint(-20, 20),
        ...
        if random.random() < 0.5:
            weapon_list = ['圣剑', '黑骑士的剑', '咖喱棒', '能量剑', '激光剑', '链刃']
            weapon = random.choice(weapon_list)
            if weapon not in player.weapons:
                player.weapons.append(weapon)
                print(f"宝箱怪掉落武器: {weapon}")
...
def enter_room(door):
    ...
    if current_room_type == 'battle':
        ...
        if random.random() < 0.15:
            chest = MimicChest(WIDTH//2, HEIGHT//2)
        else:
...
while running:
            ...
            if event.key == pygame.K_f and not game_over:
                ...
                for chest in chests:
                    if chest.rect.colliderect(player.rect) and not chest.opened:
                        # 检查是否是宝箱怪
                        is_mimic = isinstance(chest, MimicChest)
                        chest.open()
                        # 只有普通宝箱才生成掉落物，宝箱怪自己处理
                        if not is_mimic:
                ...
                while waiting:
                    ...
                    # 标题面板
                    panel_rect = pygame.Rect(WIDTH//2 - 350, 60, 700, 520)
                    pygame.draw.rect(screen, (40, 35, 60), panel_rect, border_radius=15)
                    pygame.draw.rect(screen, (100, 80, 150), panel_rect, 4, border_radius=15)
                    ...
                    # 标题
                    title = font.render("商人交易 - 按数字键购买,ESC退出", True, (255, 220, 150))
                    ...
                    for i, item in enumerate(merchant.items):
                        ...
                        # 物品背景框
                        item_box = pygame.Rect(WIDTH//2 - 300, y_pos, 600, 65)
                        ...
                        # 检查是否已拥有
                        if item.get('type') == 'weapon' and item['name'] in player.weapons:
                            bg_color = (40, 40, 40)
                            border_color = (60, 60, 60)
                        
                        pygame.draw.rect(screen, bg_color, item_box, border_radius=10)
                        pygame.draw.rect(screen, border_color, item_box, 3, border_radius=10)
                        
                        # 物品名称
                        name_text = small_font.render(f"{i+1}. {item['name']}", True, WHITE)
                        screen.blit(name_text, (WIDTH//2 - 280, y_pos + 12))
                        
                        # 价格
                        price_color = YELLOW if player.gold >= item['price'] else (200, 100, 100)
                        price_text = small_font.render(f"{item['price']} 金币", True, price_color)
                        screen.blit(price_text, (WIDTH//2 + 220 - price_text.get_width(), y_pos + 12))
                        
                        # 已拥有提示
                        if item.get('type') == 'weapon' and item['name'] in player.weapons:
                            owned_text = small_font.render("已拥有", True, GRAY)
                            screen.blit(owned_text, (WIDTH//2 - price_text.get_width() - 80, y_pos + 12))
                        ...
                        if type_text:
                            type_label = small_font.render(type_text, True, border_color)
                            screen.blit(type_label, (WIDTH//2 - 280, y_pos + 38))
    ...
    if not game_over:
        ...
        if current_room_type == 'boss':
            if len(bosses) == 0 and len(enemies) == 0 and not wave_completed:
                wave_completed = True
                room_cleared = True
        elif current_room_type == 'battle' or current_room_type == 'shop':
            if len(mobs) == 0 and len(enemies) == 0 and not wave_completed:
                wave_completed = True
            ...
            elif len(mobs) == 0 and len(enemies) == 0 and wave_completed:
                room_cleared = True
        elif current_room_type == 'treasure':
            if len(enemies) == 0 and wave_completed:
                room_cleared = True
        ...
        # 只有房间清理完成才能离开，商人Boss战期间不能离开
        for door in doors:
            can_leave = room_cleared or current_room_type == 'treasure'
            
            # 检查是否有敌对商人Boss存在，如果有就不能离开
            has_merchant_boss = False
            for enemy in enemies:
                if hasattr(enemy, 'phase'):  # 只有HostileMerchantBoss有phase属性
                    has_merchant_boss = True
                    break
            
            if has_merchant_boss:
                can_leave = False
        ...
        # 处理敌对商人（enemies组）激光伤害
        for enemy in enemies:
            if hasattr(enemy, 'laser_state') and enemy.laser_state == 'firing' and enemy.laser_damage:
                # 计算玩家是否在激光路径上
                dist_to_player = math.hypot(player.rect.centerx - enemy.rect.centerx, 
                                           player.rect.centery - enemy.rect.centery)
                laser_length = 1000
                if dist_to_player < laser_length:
                    dx = player.rect.centerx - enemy.rect.centerx
                    dy = player.rect.centery - enemy.rect.centery
                    dot_product = dx * math.cos(enemy.laser_angle) + dy * math.sin(enemy.laser_angle)
                    if dot_product > 0:
                        # 计算玩家到激光线的距离
                        cross_product = dx * math.sin(enemy.laser_angle) - dy * math.cos(enemy.laser_angle)
                        distance_to_laser = abs(cross_product)
                        if distance_to_laser < 30:  # 激光宽度
                            if not player.stealth:
                                # 商人激光伤害
                                damage = enemy.damage * 1.5 if enemy.phase == 1 else enemy.damage * 2.0
                                player.take_damage(damage)
                            # 标记已经造成伤害，避免重复伤害
                            enemy.laser_damage = False
    ...
    if offset_x != 0 or offset_y != 0:
        ...
        # 绘制敌对商人激光攻击效果（更炫酷！）
        for enemy in enemies:
            if hasattr(enemy, 'laser_state') and enemy.laser_state != 'idle':
                if enemy.laser_state == 'charging':
                    # 前摇光束效果（金色，更明显）
                    progress = enemy.laser_timer / 30
                    width = int(4 + progress * 10)
                    alpha = int(80 + progress * 150)
                    
                    charge_surface = pygame.Surface((WIDTH, HEIGHT), pygame.SRCALPHA)
                    pygame.draw.line(charge_surface, (255, 255, 0, alpha), enemy.laser_start, enemy.laser_end, width)
                    temp_surface.blit(charge_surface, (0, 0))
                elif enemy.laser_state == 'firing':
                    # 实际激光效果（金色，更宽更亮！）
                    # 最内层光晕
                    inner_glow = pygame.Surface((WIDTH, HEIGHT), pygame.SRCALPHA)
                    pygame.draw.line(inner_glow, (255, 255, 200, 80), enemy.laser_start, enemy.laser_end, 40)
                    temp_surface.blit(inner_glow, (0, 0))
                    
                    # 外层光束
                    outer_surface = pygame.Surface((WIDTH, HEIGHT), pygame.SRCALPHA)
                    pygame.draw.line(outer_surface, (255, 215, 0, 220), enemy.laser_start, enemy.laser_end, 20)
                    temp_surface.blit(outer_surface, (0, 0))
                    
                    # 中心光束（最亮）
                    center_surface = pygame.Surface((WIDTH, HEIGHT), pygame.SRCALPHA)
                    pygame.draw.line(center_surface, (255, 255, 255, 255), enemy.laser_start, enemy.laser_end, 10)
                    temp_surface.blit(center_surface, (0, 0))
